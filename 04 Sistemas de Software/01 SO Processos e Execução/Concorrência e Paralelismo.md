...
- Aparentemente similar a [[#SIMD — Instrução única, múltiplos dados|SIMD]], mas a diferença está na implementação interna (largura dinâmica, chaining, strip mining nativo)
- Exemplo clássico: **Cray-1** (1976) — computação científica; ISA de referência: **RV64V**
- Conceitos centrais: convoys, chimes, chaining, multiple lanes, scatter/gather, strip mining → [[Arquitetura Vetorial]]
...
- GPUs modernas seguem modelo SIMD — denominadas **GPGPUs** quando usadas para computação geral → [[GPU — Arquitetura SIMD]]
