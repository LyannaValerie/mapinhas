...
## Limitações vs. Arquitetura Vetorial (P&H Cap. 4)

SIMD multimídia x86 difere estruturalmente de [[Arquitetura Vetorial|arquiteturas vetoriais]] (ex.: Cray-1, RV64V):

| Aspecto | SIMD x86 (SSE/AVX) | Arquitetura Vetorial |
|---|---|---|
| Largura | **Fixa** (128/256/512 bits) | **Dinâmica** — vl ≤ MVL |
| Chaining | Não | Sim (flexible chaining) |
| Strip mining | Manual / compilador | Nativo via vl |
| Scatter/gather | Limitado (AVX2+) | Instrução dedicada |
| Predicate/mask | AVX-512 k0–k7 (tardio) | Parte da ISA desde o início |
| Compilabilidade | Difícil; uso via assembly/intrinsics | Mais fácil para compiladores vetorizadores |

Extensões recentes (AVX-512) aproximam x86 do modelo vetorial, mas largura fixa ainda exige recompilação por unidade.
...
- [[Arquitetura Vetorial]] — largura dinâmica, chaining, strip mining nativo
- [[GPU — Arquitetura SIMD]] — terceira variação: SIMD paralelo com multithreading
