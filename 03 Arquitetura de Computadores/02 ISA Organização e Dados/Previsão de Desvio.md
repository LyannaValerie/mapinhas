...
Notação: preditor (m, n) usa m bits de histórico global para indexar tabela de contadores de n bits.

## Preditor por Torneio (Tournament Predictor)

Combina preditor **global** (correlação entre branches) com preditor **local** (histórico do próprio branch), usando um **seletor** que aprende qual dos dois é mais preciso para cada branch:

```
Branch PC ──▶ [Preditor Global] ──▶ ┐
                                     ├──▶ [Seletor] ──▶ Previsão final
Branch PC ──▶ [Preditor Local]  ──▶ ┘
```

O seletor é ele mesmo um contador de saturação de 2 bits por entrada. Atualiza para "preferir global" quando global acerta e local erra, e vice-versa.

| Preditor | Força | Fraqueza |
|---|---|---|
| Global | captura correlação entre branches distintos | confusão por aliasing |
| Local | preciso em branches com padrão regular (laços) | não captura correlação cruzada |
| Torneio | aprende qual usar por branch | overhead de área (~3 tabelas) |

Usado no Core i7 920 (combinação de 2-bit + global + exit predictor de laço).

## Preditor Tagged Hybrid (TAGE)

Preditor de estado da arte. Usa múltiplas tabelas indexadas por **históricos de tamanhos crescentes** (ex: 4, 8, 16, 32, 64 ciclos):

- Cada tabela tem tag = hash(PC, histórico) para evitar aliasing
- Previsão vem da tabela com maior histórico que tem um hit
- Subsume o papel de preditor de laço, global e local em estrutura única
- Taxa de acerto >98% em benchmarks modernos

Acredita-se que o Core i7 6700 e processadores recentes Intel usem variante TAGE.
