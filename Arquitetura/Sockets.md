...
## Relações (SPO)
- Socket → é do ponto de vista do programa → arquivo aberto com [[Unix I/O|descritor de arquivo]]
...
- [[Unix I/O]] → usado para ler/escrever → socket descriptor (após conexão estabelecida)
...
Sockets são arquivos Linux — referenciados por descritores, lidos/escritos via `read`/`write` ou [[Unix I/O|RIO]].
...
## Ver também
- [[Unix I/O]] — `read`/`write`/`dup2`; RIO; compartilhamento de arquivo; `dup2` para CGI
